...
impl<'a> InitializedAudioTagAppender<'a> { 
    pub fn append_metadata(self: InitializedAudioTagAppender<'a>) -> Result<FinalizedAudioTagAppender<'a>, String> {
        let mut audio_file = match OpenOptions::new().read(true).write(true).open(self.audio_extractor.write_path()) {
        ...
        let mut current_tags = match Tag::default().read_from_path(self.audio_extractor.write_path()) {
            Ok(val) => {
                val 
            }
            Err(e) => {
        ...
        current_tags.set_genre("Electronic"); //TODO playlist name
